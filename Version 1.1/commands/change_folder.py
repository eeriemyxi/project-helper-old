    ...
    try:
        ...
        if inp != '':
            ...
            if os.path.exists(inp):
                settings['path'] = inp
                with open('settings.json', 'w') as i:
                    i.write(json.dumps(settings))
                if json.loads(open('settings.json', 'r').read())['path'] == inp:
        ...
        if isinstance(e, FileNotFoundError):
            ...
            print('An unknown error occured. Please copy the error log below and send it at mail@myxi.tk')